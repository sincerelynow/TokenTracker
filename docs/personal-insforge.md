...
个人分支构建的 macOS 和 Windows 桌面包不会连接上游 release API，也不会在启动、驻留或旧版“检查更新”动作中自动下载并安装上游包。需要升级时，通过本 fork 的 GitHub Actions 手动生成新构建：

1. 将要发布的提交推送到个人分支。
2. 打开仓库的 **Actions → build desktop apps (personal) → Run workflow**，选择分支和 `target`（`macos`、`windows` 或 `all`）；只有需要重新下载依赖时才勾选 `no_cache`。
3. 等待工作流完成，在对应的 Actions run 中下载 artifact：macOS 使用 `TokenTracker-macOS-<run number>` 中的 DMG，Windows 使用 `TokenTracker-Windows-<run number>` 中的 ZIP 或安装程序。artifact 默认保留 7 天，不会创建 GitHub Release。
4. 退出正在运行的旧桌面包，再手动安装下载的 DMG 或 Windows 安装程序；Windows ZIP 可解压到新的个人安装目录后运行。

工作流构建 Dashboard 时会读取仓库 Variables 中的 `TOKENTRACKER_INSFORGE_BASE_URL` 与 `TOKENTRACKER_INSFORGE_ANON_KEY`。请在个人 fork 中保持这两个变量指向同一个 InsForge 实例；未配置时桌面包仍可运行本地功能，但不会带有云实例目标。
