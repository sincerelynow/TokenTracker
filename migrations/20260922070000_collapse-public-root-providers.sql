-- Public community insights expose provider families, never root identifiers.
-- Private account breakdown retains distinct codex-root and dsh-root sources.
...
BEGIN
  ...
  provider_totals AS MATERIALIZED (
    SELECT
      CASE
        WHEN u.source LIKE 'codex-root:%' THEN 'codex'
        WHEN u.source LIKE 'dsh-root:%' THEN 'dsh'
        ELSE u.source
      END AS source,
