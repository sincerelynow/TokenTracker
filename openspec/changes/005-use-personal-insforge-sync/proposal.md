...
- 本地 Dashboard 从 CLI 的 `config.json` 读取个人实例 URL 与公开 anon key；配置齐全即可在“设置 → 账户”登录，无需构建时注入 `VITE_INSFORGE_*`。
...
- SC-004: 无 `VITE_INSFORGE_*` 的本地 Dashboard 构建仍可读取运行时配置并启用登录；远程托管 Dashboard 保持显式构建配置。
