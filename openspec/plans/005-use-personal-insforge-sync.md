...
approval_evidence: "用户在修订版 3 规划完成后明确回复：批准按修订版 3 核对当前实现并完成新增验证"
archive_approved_by: ""
archive_approved_at: ""
...
- 本地 Dashboard 从 CLI `config.json` 读取有效 URL/公开 anon key，设置 → 账户登录无需 `VITE_INSFORGE_*` 构建变量；远程托管 Dashboard 仍需显式构建配置。
...
6. 本地服务提供仅含 URL/公开 anon key 的无缓存配置接口；Dashboard 在认证 Provider 挂载前读取配置，刷新页面即可应用更新。
...
- `src/lib/local-api.js` — 代理仅接受个人实例，并提供仅返回公开 URL/key 的本地运行时配置接口。
...
- `dashboard/src/lib/insforge-config.ts` — 移除生产实例硬编码回退，本地读取运行时云配置，远程托管保留构建配置。
- `dashboard/src/lib/insforge-config.test.ts` — 验证运行时配置启用/禁用登录及旧服务端回退。
- `dashboard/src/main.jsx` — 认证 Provider 挂载前加载本地云配置。
...
- `dashboard/vite.config.js` — 开发代理无配置时不指向上游，并将运行时配置接口交给当前仓库本地 API。
...
- `test/local-cloud-config.test.js` — 运行时公开配置、配置变更与不完整配置的回归测试。
...
5. 逐项核对 AC-001–005；所有 required evidence 通过后标记 `ready_to_archive`。
6. 结束并等待独立归档授权；失败则记录 `blocked` 和恢复条件。

## 8. Data/API Changes

- **Database:** 个人空实例新增可复现基础 schema；按顺序应用现有 migrations/RPC，不导入旧用户或用量。
- **API:** 保持现有 `tokentracker-*` 契约；本地新增 `GET /functions/tokentracker-cloud-config`，仅返回公开 URL/key。
- **Configuration:** 云 URL/公开 anon key 显式配置；本地 Dashboard 使用 CLI `config.json`，上传 checkpoint 按实例、账号及机器隔离。
- **Environment Variables:** CLI 可用 `TOKENTRACKER_INSFORGE_BASE_URL`、`TOKENTRACKER_INSFORGE_ANON_KEY` 覆盖配置；远程托管 Dashboard 构建使用 `VITE_INSFORGE_BASE_URL`、`VITE_INSFORGE_ANON_KEY`，本地 Dashboard 不要求这些构建变量。
...
| 公开配置接口泄露私密凭据或缓存旧目标 | High | 只返回 URL/公开 anon key、禁用缓存、缺项返回空配置 |
...
- 本地配置接口与 Dashboard 运行时登录配置测试。

### Integration Test

- 临时 queue/模拟 ingest 的新实例重传、失败恢复和两次同步；Dashboard build 与 guardrails。
- 无 `VITE_INSFORGE_*` 的 Dashboard 构建、类型检查与本地配置读取验证；不执行桌面 APP 测试。
...
- [x] AC-005: 本地 `config.json` 的 URL/key 启用账户登录，缺项禁用，远程托管构建配置保持有效 — Evidence: VER-008
...
- 修订版 2 已完成并通过验证；修订版 3 的本地运行时登录配置已获批准，并在当前工作树通过新增验收。归档仍需用户单独授权。
