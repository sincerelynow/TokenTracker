...
test("public discovery surfaces describe all 36 supported tools", () => {
  ...
  assert.match(index, /Supported AI coding tools \(36\)/);
...
test("marketing logo wall includes the same 36 product integrations", () => {
  ...
  assert.equal(providers.length, 36);
  assert.equal(new Set(providers).size, 36);

  for (const provider of ["every-code", "reasonix", "kilocode", "roocode", "zed", "goose", "droid", "qoder", "anythingllm", "dsh", "prime-agent", "trae-cn", "dots", "lmstudio", "unsloth"]) {
    assert.ok(providers.includes(provider), `logo wall includes ${provider}`);
  }
});

test("CLI onboarding advertises the same 36 supported integrations", () => {
...
test("npm metadata carries the current product hook", () => {
  ...
  assert.match(pkg.description, /36 tools/);
